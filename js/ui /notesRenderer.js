...
export async function renderNotes({
  ...
  if (!notes.length) {
    ...
    fragment.appendChild(
      createEmptyStateElement(emptyMessage.title, emptyMessage.message)
    );
...
export function renderMoreNotes({
  ...
  if (refs.loadMoreBtn) {
    refs.loadMoreBtn.hidden = !state.hasMoreNotes;
  }
}
...
export function createNoteCard({
  ...
  if (attachmentZone) {
    item.appendChild(attachmentZone);
  }
...
function createNoteBadges(note) {
  const badges = createElement('div', {
    className: 'note-badges'
  });

  if (note.favorite) {
    badges.appendChild(createBadge('⭐ Favori'));
  }
...
function createAttachmentZone(note) {
  ...
  const zone = createElement('div', {
    className: 'note-attachment-zone'
  });
  ...
  const chip = createElement('span', {
    ...
    attrs: {
      title: note.fileName || 'Pièce jointe'
    }
  });
